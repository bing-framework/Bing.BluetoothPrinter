namespace Bing.BluetoothPrinter.Zicox
{
    ...
    public partial class ZicoxPrintClient
    {
        ...
        public ZicoxPrintClient Box(int x0, int y0, int x1, int y1, int width) => WriteRawLine($"BOX {x0} {y0} {x1} {y1} {width}");
        ...
        public ZicoxPrintClient Line(int x0, int y0, int x1, int y1, int width) => WriteRawLine($"L {x0} {y0} {x1} {y1} {width}");
        ...
        /// </remarks>
        public ZicoxPrintClient InverseLine(int x0, int y0, int x1, int y1, int width) => WriteRawLine($"IL {x0} {y0} {x1} {y1} {width}");
        ...
        /// </param>
        /// <returns></returns>
        public ZicoxPrintClient Pattern(int number) => WriteRawLine($"PATTERN {number}");
        ...
        /// </remarks>
        public ZicoxPrintClient ExpandedGraphics(int width, int height, int x, int y, string data) => WriteRawLine($"EG {width} {height} {x} {y} {data}");
        ...
        /// 可以使用图形命令打印位映射图形。扩展图形数据使用 ASCII 十六进制字符来表示（参见示例）。通过对十六进制 数据的等效二进制字符使用 COMPRESSED-GRAPHICS 命令，可以将数据大小减半。如果使用 CG，对于每 8 位图 形数据，将会发送一个 8 位字符。如果使用 EG，将使用两个字符（16 位）来传输 8 位图形数据，因此 EG 的效率会 减半。   但是由于该数据是字符数据，因此比二进制数据更容易处理和传输。 
        /// </remarks>
        public ZicoxPrintClient VExpandedGraphics(int width, int height, int x, int y, string data) => WriteRawLine($"VEG {width} {height} {x} {y} {data}");
        ...
        /// 可以使用图形命令打印位映射图形。扩展图形数据使用 ASCII 十六进制字符来表示（参见示例）。通过对十六进制 数据的等效二进制字符使用 COMPRESSED-GRAPHICS 命令，可以将数据大小减半。如果使用 CG，对于每 8 位图 形数据，将会发送一个 8 位字符。如果使用 EG，将使用两个字符（16 位）来传输 8 位图形数据，因此 EG 的效率会 减半。   但是由于该数据是字符数据，因此比二进制数据更容易处理和传输。 
        /// </remarks>
        public ZicoxPrintClient CompressedGraphics(int width, int height, int x, int y, string data) => WriteRawLine($"CG {width} {height} {x} {y} {data}");
        ...
        /// 可以使用图形命令打印位映射图形。扩展图形数据使用 ASCII 十六进制字符来表示（参见示例）。通过对十六进制 数据的等效二进制字符使用 COMPRESSED-GRAPHICS 命令，可以将数据大小减半。如果使用 CG，对于每 8 位图 形数据，将会发送一个 8 位字符。如果使用 EG，将使用两个字符（16 位）来传输 8 位图形数据，因此 EG 的效率会 减半。   但是由于该数据是字符数据，因此比二进制数据更容易处理和传输。 
        /// </remarks>
        public ZicoxPrintClient VCompressedGraphics(int width, int height, int x, int y, string data) => WriteRawLine($"VCG {width} {height} {x} {y} {data}");
