namespace Bing.BluetoothPrinter.Zicox
{
    ...
    public partial class ZicoxPrintClient
    {
        ...
        public ZicoxPrintClient Contrast(int level) => WriteRawLine($"CONTRAST {level}");
        ...
        public ZicoxPrintClient Tone(int level) => WriteRawLine($"TONE {level}");
        ...
        public ZicoxPrintClient Center(int? end = null) => WriteRawLine($"CENTER{(end == null ? string.Empty : $" {end}")}");

        /// <summary>
        /// 对齐命令 - 左对齐
        /// </summary>
        /// <remarks>使用对齐命令可以控制字段的对齐方式。默认情况下，打印机将左对齐所有字段。对齐命令将对所有后续字段保持 有效，直至指定了其他对齐命令。</remarks>
        public ZicoxPrintClient Left() => WriteRawLine("LEFT");
        ...
        /// <param name="end">对齐的结束点。如果未输入参数，则对于横向打印，对齐命令将使用打印头的宽度；而对于纵向打印，对 齐命令将使用零（页头）。 </param>
        /// <remarks>使用对齐命令可以控制字段的对齐方式。默认情况下，打印机将左对齐所有字段。对齐命令将对所有后续字段保持 有效，直至指定了其他对齐命令。</remarks>
        public ZicoxPrintClient Right(int? end = null) => WriteRawLine($"RIGHT{(end == null ? string.Empty : $" {end}")}");
        ...
        public ZicoxPrintClient Wait(int delayTime) => WriteRawLine($"WAIT {delayTime}");

        /// <summary>
        /// REWIND 命令（开启）
        /// </summary>
        /// <remarks>此命令用于打开或关闭回卷（或卷纸）电机。打印机在开机时默认为 REWIND-ON。如果打印机没有配备电机驱动 的回卷机制，则将忽略 REWIND 命令。 </remarks>
        public ZicoxPrintClient RewindOn() => WriteRawLine("REWIND-ON");

        /// <summary>
        /// REWIND 命令（关闭）
        /// </summary>
        /// <remarks>此命令用于打开或关闭回卷（或卷纸）电机。打印机在开机时默认为 REWIND-ON。如果打印机没有配备电机驱动 的回卷机制，则将忽略 REWIND 命令。 </remarks>
        public ZicoxPrintClient RewindOff() => WriteRawLine("REWIND-OFF");
        ...
        public ZicoxPrintClient TensionPre(int length) => WriteRawLine($"PRE-TENSION {length}");
        ...
        /// <param name="length">回卷电机用于收紧衬纸张力的单位长度。当张力调整到位后，回卷电机将滑动（不会为下一个打印周期 用尽所有调整余地）。</param>
        /// <remarks>TENSION 命令用于在打印标签之前和/或之后，通过按预先指定的长度运行回卷电机来调整衬纸张力。对于配备 电机驱动的回卷机制的打印机，此调整操作可以改善剥离器的性能。如果打印机没有配备电机驱动的回卷机制，则将 忽略 TENSION 命令。 </remarks>
        public ZicoxPrintClient TensionPost(int length) => WriteRawLine($"POST-TENSION {length}");
        ...
        public ZicoxPrintClient Speed(int level) => WriteRawLine($"SPEED {level}");
        ...
        public ZicoxPrintClient SetSp(int spacing) => WriteRawLine($"SETSP {spacing}");

        /// <summary>
        /// UNDERLINE 命令（开启）
        /// </summary>
        /// <remarks>UNDERLINE 命令用于给文本加下划线。仅当所使用的字体支持下划线时，此命令才会起作用。如果所使用的字体不支 持 UNDERLINE，则将忽略此命令。</remarks>
        public ZicoxPrintClient UnderlineOn() => WriteRawLine("UNDERLINE ON");

        /// <summary>
        /// UNDERLINE 命令（关闭）
        /// </summary>
        /// <remarks>UNDERLINE 命令用于给文本加下划线。仅当所使用的字体支持下划线时，此命令才会起作用。如果所使用的字体不支 持 UNDERLINE，则将忽略此命令。</remarks>
        public ZicoxPrintClient UnderlineOff() => WriteRawLine("UNDERLINE OFF");
        ...
        public ZicoxPrintClient OnOutOfPaper(string action, int retries) => WriteRawLine($"ON-OUT-OF-PAPER {action} {retries}");
        ...
        public ZicoxPrintClient OnFeed(string action) => WriteRawLine($"ON-FEED {action}");
        ...
        public ZicoxPrintClient PreFeed(int length) => WriteRawLine($"PREFEED {length}");
        ...
        public ZicoxPrintClient PostFeed(int length) => WriteRawLine($"POSTFEED {length}");
        ...
        public ZicoxPrintClient PreSentAt(int length, int delay) => WriteRawLine($"PRESENT-AT {length} {delay}");
        ...
        public ZicoxPrintClient Country(string name) => WriteRawLine($"COUNTRY {name}");
        ...
        /// 使用“\\”（双反斜杠）作为数据的占位符。 
        /// </remarks>
        public ZicoxPrintClient DefineFormat(string name, params string[] data)
        {
        ...
        public ZicoxPrintClient UseFormat(string name, params object[] data)
        {
        ...
        public ZicoxPrintClient Beep(int length) => WriteRawLine($"BEEP {length}");

        /// <summary>
        /// CUT 命令
        /// </summary>
        /// <remarks>在配备有切纸器的打印机上，使用此命令可在打印标签之后裁切标签。</remarks>
        public ZicoxPrintClient Cut() => WriteRawLine("CUT");

        /// <summary>
        /// PARTIAL-CUT 命令
        /// </summary>
        /// <remarks>在配备有切纸器的打印机上，使用此命令可在打印标签之后裁切标签，保留部分标签不进行裁切以便可以轻松撕 开标签剩余部分。</remarks>
        public ZicoxPrintClient PartialCut() => WriteRawLine("PARTIAL-CUT");
        ...
        public ZicoxPrintClient CutAt(int length) => WriteRawLine($"CUT-AT {length}");
    }
}
