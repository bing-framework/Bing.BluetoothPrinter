namespace Bing.BluetoothPrinter.Zicox
{
    ...
    public partial class ZicoxPrintClient
    {
        ...
        /// </remarks>
        public ZicoxPrintClient Pdf417(int x, int y, int width, int height, int col, int safeLevel, params string[] data)
        {
        ...
        /// 堆叠的数量和高度由用户控制。这类条码可以包含整个 ASCII 255 字符集，并能使用不同的编码方案和不同的纠错安全级别。大数据编码量为 2725 个字符
        /// </remarks>
        public ZicoxPrintClient VPdf417(int x, int y, int width, int height, int col, int safeLevel, params string[] data)
        {
        ...
        public ZicoxPrintClient Maxicode(int x, int y, params (string tag, string data)[] param)
        {
        ...
        public ZicoxPrintClient QRCode(int x, int y, int model, int widthWithHeight, params string[] data)
        {
        ...
        /// <param name="data">提供生成 QR Code 所需的信息。需要录入二维自定义信息</param>
        // ReSharper disable once InconsistentNaming
        public ZicoxPrintClient VQRCode(int x, int y, int model, int widthWithHeight, params string[] data)
        {
        ...
        // ReSharper disable once InconsistentNaming
        public ZicoxPrintClient QRCode(int x, int y, int model, int widthWithHeight, char errorCorrectionLevel, int? maskNo, string data)
        {
            WriteRawLine($"B QR {x} {y} M {model} U {widthWithHeight}");
        ...
        /// <param name="data">二维码信息</param>
        // ReSharper disable once InconsistentNaming
        public ZicoxPrintClient VQRCode(int x, int y, int model, int widthWithHeight, char errorCorrectionLevel, int? maskNo, string data)
        {
            WriteRawLine($"VB QR {x} {y} M {model} U {widthWithHeight}");
            WriteRawLine($"{errorCorrectionLevel}{(maskNo == null ? string.Empty : maskNo.ToString())}A,{data}");
            WriteRawLine("ENDQR");
        ...
        // ReSharper disable once InconsistentNaming
        public ZicoxPrintClient QRCodeManual(int x, int y, int model, int widthWithHeight, char errorCorrectionLevel, int? maskNo, params (string charMode, string data)[] param)
        {
            WriteRawLine($"B QR {x} {y} M {model} U {widthWithHeight}");
        ...
        /// <param name="param">参数：字符模式符(N、A、Bxxxx、K)）-数据</param>
        // ReSharper disable once InconsistentNaming
        public ZicoxPrintClient VQRCodeManual(int x, int y, int model, int widthWithHeight, char errorCorrectionLevel, int? maskNo, params (string charMode, string data)[] param)
        {
            WriteRawLine($"VB QR {x} {y} M {model} U {widthWithHeight}");
            WriteRaw($"{errorCorrectionLevel}{(maskNo == null ? string.Empty : maskNo.ToString())}M");
            foreach (var item in param)
        ...
        /// <param name="data">条码数据</param>
        /// <returns></returns>
        public ZicoxPrintClient Aztec(int x, int y, int width, int errorCorrectionPercentage, params string[] data)
        {
        ...
        /// <param name="data">条码数据</param>
        /// <returns></returns>
        public ZicoxPrintClient VAztec(int x, int y, int width, int errorCorrectionPercentage, params string[] data)
        {
